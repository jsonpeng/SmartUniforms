...
class FlashSaleController extends AppBaseController
{
    ...
    public function __construct(FlashSaleRepository $flashSaleRepo,ProductRepository $productRepo,SpecProductPriceRepository $specProductPriceRepo,TeamSaleRepository $teamSaleRepo)
    {
    ...
    public function index(Request $request)
    {
        ...
        //if($created_desc_status){
            $flashSales=$flashSales->orderBy('created_at','desc');
        //}
        $flashSales=$flashSales->paginate($this->defaultPage());
    ...
    private function progressFlashSale($inputs, $product_spec)
    {
        ...
        if($product_spec_arr[1] == "0") {
            ...
            if(!empty($product_spec)) {
                ...
                $inputs['image'] = empty($product_spec->image) ?$product->image: $product_spec->image;
            }
        }
        return $inputs;
    }
