...
class OrderController extends AppBaseController
{
    ...
    public function __construct(OrderRepository $orderRepo, OrderActionRepository $orderActionRepo)
    {
    ...
    public function index(Request $request)
    {
       ...
       // return $input;
        $orders = $orders->where('price','>',0)->paginate($this->defaultPage());
    ...
    public function show($id)
    {
        ...
        //app('commonRepo')->generateCode($order);

        $items = $order->items()->get();
        $actions=$order->actions()->orderBy('created_at','desc')->paginate($this->defaultPage());
    
        return view('admin.orders.show', compact('order', 'items','actions'));
    }
    ...
    public function update($id, UpdateOrderRequest $request)
    {
        ...
        $inputs = $request->all();
        /*
        if (array_key_exists('delivery_company', $inputs) && array_key_exists('delivery_no', $inputs)) {
            $inputs['order_delivery'] = '已发货';
        }
        */
        $action=[];
        $action['order_id']=$id;
        ...
        if(array_key_exists('backup01', $inputs)){
            $action['action']="修改备注信息";
            $action['status_desc'] = "修改备注信息为" . $inputs['backup01'];
        ...
        if(array_key_exists('add_product', $inputs)){
            ...
            if(!empty($product)) {
                ...
                $action['status_desc'] = "添加商品" . $product->name;
            }
        }
        ...
        if(array_key_exists('del_product',$inputs)){
            ...
            if(!empty($product)) {
                $this->orderRepository->reCaculateOrderPrice($id);
                $action['status_desc'] = "删除商品" . $product->name;
            }
        }

        if(array_key_exists('delivery_company',$inputs)){
            $action['action']="配送信息修改";
            $action['status_desc']="修改快递公司为".$inputs['delivery_company'].",修改快递单号为".$inputs['delivery_no'];
        }

        if(array_key_exists('order_pay', $inputs)){
            $action['action']="支付状态修改";
            $action['status_desc']=is_null($inputs['operation_log'])?'无':$inputs['operation_log'];
            //设置订单支付时间
        ...
        if(array_key_exists('customer_name', $inputs)){
            $action['action']="收货地址修改";
            $action['status_desc']="修改收货人为".$inputs['customer_name'];
            $action['status_desc']=$action['status_desc'].",修改收货电话为".$inputs['customer_phone'];
            $action['status_desc']=$action['status_desc'].",修改地址为".$inputs['customer_address'];
        }

        if (array_key_exists('freight', $inputs)) {
            $inputs['price'] = $order->price - $order->freight + $inputs['freight'];
            $action['action']="运费调整";
            $action['status_desc']="调整运费为".$inputs['freight'];

        }

        if (array_key_exists('price_adjust', $inputs)) {
            $inputs['price'] += $inputs['price_adjust'] - $order->price_adjust;
            $action['action']=$action['action'].",调整价格";
            $action['status_desc']= $action['status_desc'].",调整价格为".$inputs['price_adjust'];
        }

        $order = $this->orderRepository->update($inputs, $id);
        if(!empty($order)) {
            $action['order_status'] = $order->order_status;
            $action['shipping_status'] = $order->order_delivery;
            $action['pay_status'] = $order->order_pay;
            $this->orderActionRepository->saveAction($action);
        }
