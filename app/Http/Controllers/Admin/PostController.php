                ...
                if (is_array($v) && $key != 'categories' ){
                    if(strpos($input['type'],",") !==false){
                        ...
                        foreach ($type as $item){
                            $s_type=$this->customPostTypeRepository->getNameBySlug($item);
                            if(!empty($s_type)){
                                $this->postRepository->update(['type'=>$item],$post->id);
                            }
                        }
                        $customPostItemsSlug=$this->customPostTypeItemsRepository->getOneSlugByName($key);
                        if(in_array($customPostItemsSlug,$type)){
                            $customPostItems= $this->customPostTypeItemsRepository->getOneByName($key);
                            if($customPostItems->type=='textarea'){
                                $v[0]= preg_replace("/\n|\r\n/", "<br/>",$v[0]);
                            }
                            $this->postItemsRepository->create(['key'=>$key,'name'=>$customPostItems->des,'value'=>$v[0],'post_id'=>$post->id]);
                        }
                    }else{
                        $customPostItemsSlug=$this->customPostTypeItemsRepository->getOneSlugByName($key);
                        if($input['type']==$customPostItemsSlug){
                            $customPostItems= $this->customPostTypeItemsRepository->getOneByName($key);
                            if($customPostItems->type=='textarea'){
                                $v[0]= preg_replace("/\n|\r\n/", "<br/>",$v[0]);
                            }
            ...
            foreach ($input as $key => $v) {
                if (is_array($v) && $key != 'categories' ){
                    if(strpos($input['type'],",") !==false){
                        ...
                        if(in_array($this->customPostTypeItemsRepository->getOneSlugByName($key),$type)){
                            $this->postItemsRepository->updateByPostIdAndKey($id,$key,['key'=>$key,'name'=>$this->customPostTypeItemsRepository->getOneByName($key)->des,'value'=>$v[0]]);
                        }
                    }else{
                        if($input['type']==$this->customPostTypeItemsRepository->getOneSlugByName($key)){
                            $this->postItemsRepository->updateByPostIdAndKey($id,$key,['key'=>$key,'name'=>$this->customPostTypeItemsRepository->getOneByName($key)->des,'value'=>$v[0]]);
                        }
                    }
                }
                ...
                if (is_array($v) && $key != 'categories' ){
                    if(strpos($input['type'],",") !==false){
                        ...
                        if(in_array($customPostItemsSlug,$type)){
                            ...
                            $this->postItemsRepository->updateByPostIdAndKey($id,$key,['key'=>$key,'name'=>$customPostItems->des,'value'=>$v[0]]);
                        }
                    }else{
                        $customPostItemsSlug=$this->customPostTypeItemsRepository->getOneSlugByName($key);
                        if($input['type']==$customPostItemsSlug){
