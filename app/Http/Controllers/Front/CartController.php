...
class CartController extends Controller
{
    ...
    public function check(Request $request)
    {
        ...
        $total = ShoppingCart::total();
        $freight = app('commonRepo')->freight($address);
    ...
    private function addToCart($inputs){
        ...
        if (array_key_exists('specPriceItemId', $inputs) && $inputs['specPriceItemId'] > 0) {
            ...
            if ($items->count()) {
                $item = $items->first();
                $newQty = $this->maxCanBuy($product, $inputs['count'] + $item->qty, $specPrice->id);
                ShoppingCart::update($item->__raw_id, ['qty' => $newQty]  );
            } else {
                $newQty = $this->maxCanBuy($product, $inputs['count'], $specPrice->id);
                ShoppingCart::add(
            ...
            if ($items->count()) {
                $item = $items->first();
                $newQty = $this->maxCanBuy($product, $inputs['count'] + $item->qty);
                ShoppingCart::update($item->__raw_id, ['qty' => $newQty]);
            } else {
                $newQty = $this->maxCanBuy($product, $inputs['count']);
                ShoppingCart::add(
    ...
    public function update(Request $request)
    {
        ...
        if ($items->count()) {
            ...
            if (empty($specPrice)) {
                $newQty = $this->maxCanBuy($product, $qty);
            } else {
                $newQty = $this->maxCanBuy($product, $qty, $specPrice->id);
            }
    ...
    private function shoppingCartItems(){
        $items = ShoppingCart::all();
        //如果是带规格的就要加上规格
        foreach ($items as $item) {
    ...
    private function maxCanBuy($product, $qty, $spec_id = null)
    {
