...
class CategoryRepository extends BaseRepository
{
    ...
    {
        return Cache::remember('all_cats', Config::get('web.cachetime'), function(){
    ...
    public function getFirstRootCat()
    {
        return Cache::remember('getFirstRootCat', Config::get('web.cachetime'), function() {
            return Category::where('parent_id', 0)->orderBy('sort', 'desc')->first();
        });
    }
    ...
    //获取所有商品根分类信息
    public function getRootCategoriesCached(){
        return Cache::remember('get_root_product_categories', Config::get('web.cachetime'), function(){
            return  Category::where('parent_id', 0)->orderBy('sort', 'asc')->get();
        });
    }
    ...
    public function getRecommendCategoriesCached(){
        return Cache::remember('get_recommend_product_categories', Config::get('web.cachetime'), function() {
            return  Category::where('recommend', 1)->orderBy('sort', 'asc')->get();
        });
    }
    ...
    public function getChildCategories($parent_id){
        return  Category::where('parent_id', $parent_id)->orderBy('sort', 'asc')->get();
    }
    ...
    */
    ...
    public function getRootCatArray($id = 0){
        $catArray = array(0 => '请选择分类');
        $categories = Category::select('id', 'name')->where('parent_id', $id)->get()->toArray();
        while (list($key, $val) = each($categories)) {
            if ($id != $val['id']) {
                $catArray[$val['id']] = $val['name'];
            }
