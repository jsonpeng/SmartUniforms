...
class OrderRepository extends BaseRepository
{
    ...
    public function createOrder($user, $items, $inputs)
    {
        ...
        if (array_key_exists('coupon_id', $inputs)) {
            $finalResult = app('commonRepo')->CouponPreference($inputs['coupon_id']);
    ...
    public function cancelOrder($id, $reason='无')
    {
        try {
    ...
    */
    ...
    public function autoConfirmOrder()
    {
        ...
        ->where(Carbon::parse('sendtime')->addDays($autoConfirmDays), '<', Carbon::now())->get();
    ...
    public function clearExpiredOrder()
    {
        ...
        ->where(Carbon::parse('created_at')->addHours($hours), '<', Carbon::now())->get();
        foreach ($orders as $order) {
