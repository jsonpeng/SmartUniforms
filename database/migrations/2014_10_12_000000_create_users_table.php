...
class CreateUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->tinyInteger('email_validated')->nullable()->default(0)->comment('是否认证邮箱 0 否 1 是');
            ...
            //$table->enum('type', ['管理员','用户'])->default('用户')->comment('账号类型');
            ...
            //$table->integer('underling_number')->default(0)->comment('用户下线总数');
            
            $table->float('user_money')->default(0)->comment('用户金额');
            //$table->float('frozen_money')->default(0)->comment('冻结金额');
