...
class RolePemessionSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $team_sales_set=Permission::create(['name'=>'memberCards.*','slug'=>'memberCards', 'display_name'=>'积分卡', 'description'=>'所有页面和操作','tid'=>19,'model'=>'积分卡']);

        $team_sales_set=Permission::create(['name'=>'cardLogs.*','slug'=>'cardLogs', 'display_name'=>'积分记录', 'description'=>'所有页面和操作','tid'=>19,'model'=>'积分卡']);



        $super_admin->perms()->sync(array(
           $pageItems_set->id,
           $customPageTypes_set->id,
           $pages_set->id,
           $customPostTypeItems_set->id,
           $customPostTypes_set->id,
           $article_set->id,
           $article_cat_set->id,
           $withDrawls->id,
           $bannerItems_set->id,
           $customer_service_set->id,
           $single_page_set->id,
           $product_word_list->id,
           $product_list_alllow->id,
           $order_refundMoneys->id,
           $coupons_auto_rules->id,
           $distributions_set->id,
           $distributionLogs_set->id,
           $cities_set->id,
           $freightTems_set->id,
           $perm_add->id,
           $perm_store->id,
