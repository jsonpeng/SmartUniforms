...
<div class="form-group col-sm-12 category_select" @if (empty($coupon) || $coupon->
    ...
    {!! Form::label('category_id', '上级分类:') !!}
    <div class="row">
        <div class="col-sm-4 col-xs-4 pr0-xs">
            <?php $level1 = empty($level01) ? null : $level01; ?>
            {!! Form::select('level01', $categories, $level1  , ['class' => 'form-control level01']) !!}
        </div>
        <div class="col-sm-4 col-xs-4 pr0-xs">
            <?php $level2 = empty($level02) ? null : $level02; ?>
            {!! Form::select('level02', $level02Selections, $level2 , ['class' => 'form-control level02']) !!}
        </div>
        <div class="col-sm-4 col-xs-4 ">
            <?php $level3 = empty($level03) ? null : $level03; ?>
            {!! Form::select('level03', $level03Selections, $level3 , ['class' => 'form-control level03']) !!}
        </div>
    </div>
</div>
...
<div id="product_items" class="col-sm-12" style="display:{!! empty($coupon) || $coupon->
    ...
    <div  class="box-body" >
        ...
        <div class="items row" id="item_row_{!! $item->
            id !!}" style="border-bottom: 1px solid #f4f4f4" data-id="{!! $item->id !!}" >
        ...
        <div class="items row" id="item_row_{!! $item->
            ...
            <div id="item_form_{{ $item->
                id }}">
                <div class="col-md-4 col-xs-4">
                    <img src="{{ $item->pic }}" alt="">{{ $item->product()->first()->name }}</div>
                <div class="col-md-4 col-xs-5">{{ $item->key_name }}</div>
                ...
                <div class="col-md-2 col-xs-2">
                    <div class='btn-group' id="item_" style="padding: 8px;">
                        <a href="javascript:;" class='btn btn-danger btn-xs' id="item_delete_{{ $item->
                            id }}"  onclick="delItem(this,{{ $item->id }})">
                            <i class="glyphicon glyphicon-trash" title="确认"></i>
                        </a>
                    </div>
                </div>
                <input type="hidden" name="product_spec[]" value="{{ $item->product_id }}_{{ $item->id }}" /></div>
        </div>
        @endforeach
        @endif
