...
$('input:radio[name=use_default]').click(function(){
	if(parseInt($(this).val())==1){
		...
		if(status){
				...
				$('#freight_tems_tbody'+type).append('<tr class="all_area"><td>全国<input name="area_list[]" type="hidden" value="全国"><input type="hidden" name="area_ids_list[]" value="1"></td><td><input name="freight_first_count[]" >'+type_num+'</td><td><input name="the_freight[]">元</td><td><input name="freight_continue_count[]">'+type_num+'</td><td><input name="freight_continue_price[]">元</td><td><button class="btn btn-danger btn-xs" type="button" onclick="del_freight_item(this)"><i class="glyphicon glyphicon-trash"></i>删除</button></td></tr>');
			}else{
				$('.all_area').remove();
			}
	}else{
...
$('.add_freight_cities').click(function(){
		...
		$('#freight_tems_tbody'+type).append('<tr><td><input name="area_list[]" onclick="select_area(this)"><input type="hidden" name="area_ids_list[]" ></td><td><input name="freight_first_count[]" >'+type_num+'</td><td><input name="the_freight[]">元</td><td><input name="freight_continue_count[]">'+type_num+'</td><td><input name="freight_continue_price[]">元</td><td><button class="btn btn-danger btn-xs" type="button" onclick="del_freight_item(this)"><i class="glyphicon glyphicon-trash"></i>删除</button></td></tr>');
});
