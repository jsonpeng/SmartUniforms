        ...
        <div class="box-body">
            <div class="row">
                <form id="address">
                    ...
                    <div class="col-md-8">
                        <span class="showAddress">{!! $order->customer_address !!}</span>
                        <input type="text" class="form-control address" name="customer_address" value="{{ $order->customer_address }}"></div>
            ...
            <div class="row">
                <form id="delivery">
                    <div class="col-md-2">
                        ...
                        <input type="text" class="form-control delivery" name="delivery_company" value="{{ $order->delivery_company }}" placeholder="快递公司"></div>
                    <div class="col-md-5">
    ...
    <div class="box box-solid">
        ...
        <div class="box-body">
            <div class="row" style="font-weight: bold;">
                <div class="col-md-2 col-xs-4" >操作者</div>
                <div class="col-md-2 col-xs-3 hidden-xs" >操作时间</div>
                <div class="col-md-1 col-xs-3" >状态</div>
                <div class="col-md-1 col-xs-3 hidden-xs" >付款</div>
                <div class="col-md-1 col-xs-1 hidden-xs" >发货</div>
                <div class="col-md-2 col-xs-2 hidden-xs" >描述</div>
                <div class="col-md-3 col-xs-5" >备注</div>
            </div>
            @foreach($actions as $item)
            <div class="row">
                <div class="col-md-2 col-xs-4">{{ $item->user }}</div>
                <div class="col-md-2 col-xs-3 hidden-xs">{{ $item->created_at }}</div>
                <div class="col-md-1 col-xs-3">{{ $item->order_status }}</div>
                <div class="col-md-1 col-xs-3 hidden-xs">{{ $item->pay_status }}</div>
                <div class="col-md-1 col-xs-1 hidden-xs">{{ $item->shipping_status }}</div>
                <div class="col-md-2 col-xs-2 hidden-xs">{{ $item->action }}</div>
                <div class="col-md-3 col-xs-5">{{ $item->status_desc }}</div>
            </div>
            @endforeach
        </div>
        <!-- /.box-body -->
        <div class="tc">
            <?php echo $actions->appends("")->render(); ?></div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function addProductMenu(){
            ...
            $.ajax({
            url:"/zcjy/products/ajaxGetProductList" ,
            type:"POST",
            success:function(data){
            if(data.code==0){
                console.log(data.message);
                var products_list=data.message;
                var html_list="";
                for(var i=0;i<products_list.length;i++){
                    html_list +='<div class="productlist row" style="border-bottom:1px solid #f4f4f4;" data-status="false" data-productid="'+products_list[i].product_id+'" onclick="addProductApi(this,'+products_list[i].id+','+products_list[i].product_id+')"><div class="products_list_checkbox col-md-1"></div><div class="col-md-1" id="spec_id">'+products_list[i].id+'</div><div class="col-md-3" id="product_content"><img src="" alt="">三星S8</div><div class="col-md-2" id="spec_key_name">'+products_list[i].key_name+'</div><div class="col-md-1" id="item_count_1">'+products_list[i].inventory+'<span></span> <input type="text" class="form-control input-sm" name="count" value="1" style="display:none"></div><div class="col-md-1" id="item_price_1"><span>'+products_list[i].price+'</span> <input type="text" class="form-control input-sm" name="price" value="4500" style="display:none"></div><div class="col-md-1">'+products_list[i].price+'</div><div class="col-md-2"></div></div>';
                }

                layer.open({
                  type: 1,
                  skin: 'layui-layer-rim', //加上边框
                  area: ['1080px', '680px'], //宽高
                  content: '<div class="box-body"><div class="row" style="background-color:#edfbf8;"><div class="col-md-1"><a href="javascript:;" style="text-align:center;" onclick="varifyProductList()">确定</a></div><div class="col-md-1">编号</div><div class="col-md-3">商品</div><div class="col-md-2">规格</div><div class="col-md-1">库存</div><div class="col-md-1">单价</div><div class="col-md-1">总价</div></div>'+html_list+'</div>'
                });
                ...
                $('.productlist').each(function(){
                    var product_id=$(this).data('productid');
                       ...
                       $.ajax({
                        url:"/zcjy/products/getSingleProductById/"+product_id,
                        type:"POST",
                        success:function(data){
                            if(data.code==0){
                                  console.log($(this).find('#product_content'));
                                  var product=data.message;
                                $(this).find('#product_content').html('<img src="'+product.image+'" alt="">'+product.name+'');
                            }
                        }
                    });        
                });
