<table class="table table-responsive" id="products-table">
    ...
    @foreach($products as $product)
        <tr>
            ...
            <td>
                {!! Form::open(['route' => ['products.sdestroy', $product->id], 'method' => 'delete']) !!}
                <div class='btn-group'>
                    ...
                    <a href="{!! route('products.sedit', [$product->id]) !!}" class='btn btn-default btn-xs'><i class="glyphicon glyphicon-edit"></i></a>
      ...
      <?php $items=$product->specs;?>
        @if(count($items)>0)
          <!--有商品规格的商品-->
            ...
            <?php $consult = getConsultNum($product_item->name,$item->key_name);?>
            <tr>
                <td>@if($consult['zengdings'] == '0' || !empty($consult['zengdings']) || $consult['tuihuis'] == '0' || !empty($consult['tuihuis']))<span style="color: red;">[征订数量:{!! $consult['zengdings'] !!},退回数量:{!! $consult->tuihuis !!}]</span>@endif</td>
                ...
                {{-- <td>
                    ...
                    <div class='btn-group'>
                        ...
                        <a href="{!! route('products.edit', [$product_item->id]) !!}" class='btn btn-default btn-xs'><i class="glyphicon glyphicon-edit"></i></a>
                         {!! Form::button('<i class="glyphicon glyphicon-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-danger btn-xs', 'onclick' => "return confirm('将会删除规格相关的所有校服,确定要删除吗?')"]) !!}
                    </div>
                       {!! Form::close() !!}
                </td> --}}
            </tr>
