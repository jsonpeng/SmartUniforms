...
//选择校服
Route::get('/searchSchoolsFrame','Admin\ProductController@searchSchoolsFrame');

Route::group([ 'middleware' => ['auth.admin:admin'], 'prefix' => 'zcjy', 'namespace' => 'Admin'], function () {

	//激活码统一价格
	Route::get('com/regCodes', 'RegCodeController@com');
	Route::post('regCodes/com', 'RegCodeController@comAction')->name('regCode.com');
	//激活码
	Route::resource('regCodes', 'RegCodeController');
	//读卡器记录
	Route::resource('cardRecords', 'CardRecordController');
	//处理记录
	Route::post('dealForm/{id}','ConsultRecordController@dealForm')->name('consultRecords.dealForm');
	//导出征询单记录
	Route::post('exportSchoolsAllSpecs','ProductController@exportSchoolsAllSpecs')->name('products.exportS');
	//导出征询单记录
	Route::post('exportSchoolsAllConsults','ProductController@exportAllConsults')->name('products.exportC');
	Route::get('getSchoolExcel','ProductController@getSchoolExcel');
	//校服管理
	Route::get('schoolClothesIndex','ProductController@schoolClothesIndex')->name('products.sindex');
	Route::get('schoolClothesCreate','ProductController@schoolClothesCreate')->name('products.screate');
	Route::get('schoolClothesEdit/{id}','ProductController@schoolClothesEdit')->name('products.sedit');
	//存储操作
	Route::post('schoolClothesStore','ProductController@sstore')->name('products.sstore');
	//更新操作
	Route::patch('schoolClothesUpdate/{id}','ProductController@supdate')->name('products.supdate');
	//删除操作
	Route::delete('schoolClothesDelete/{id}','ProductController@sdestroy')->name('products.sdestroy');
	...
	//积分修改
	Route::post('ajax/user/{user_id}/credits_change','UserController@updateUserCredits');
	//余额修改
	Route::post('ajax/user/{user_id}/money_change','UserController@updateUserMoney');
	//产品分类
	...
	Route::post('ajax/set_theme_color', 'SettingController@postThemeSetting');
    ...
    //根据城市id返回对应的运费模板信息
    Route::get('ajax/getfreighttem/cid/{cid}','CitiesController@GetFreightTemByCid');
    //地区对应的模板信息
    Route::get('cities/frame/freighttem/{cid}','CitiesController@GetFreightTemByCidFrame')->name('cities.freighttem.frame');

    Route::post('ajax/wordset/{id}','ProductController@wordsListUpdate');
    //为当前管理员赋予权限
    Route::post('ajax/perm/{id}/add','PermissionsController@addPermToAdmin');
    //为当前管理员移除权限
    Route::post('ajax/perm/{id}/del','PermissionsController@delPermToAdmin');
    //冻结用户
    Route::post('ajax/freezeuser/{userid}','UserController@freezeUserById');
    //操作分销资格
    Route::post('ajax/distributeUser/{userid}','UserController@distributeUser');
    //单页面
    ...
    Route::get('all_del/memberCards','MemberCardController@allDestroy');
    
	Route::resource('memberCards', 'MemberCardController');

	Route::resource('cardLogs', 'CardLogController');
	...
	Route::resource('machineIdes', 'MachineIdeController');
	
	Route::post('cardrecord/{id}/report','CardRecordController@report')->name('cardRecord.report');

	//学校管理
	Route::resource('schools', 'SchoolController');
	//班级管理
	Route::resource('{school_id}/schoolClasses', 'SchoolClassController');
	
	//征询单记录
	Route::resource('consultRecords', 'ConsultRecordController');
});
